		...
		List<MethodInfo> PatchWithAttributes(ref MethodBase lastOriginal)
		{
			...
			foreach (var patchMethod in patchMethods)
			{
				...
				if (lastOriginal is null)
				{
                    ...
                    {
						instance.patched[patchMethod.info.method] = false;
						continue;
					}
					else throw new ArgumentException($"Undefined target method for patch method {patchMethod.info.method.FullDescription()}");
				}
				else instance.patched[patchMethod.info.method] = true;
				var job = jobs.GetJob(lastOriginal);
				job.AddPatch(patchMethod);
			}
			foreach (var job in jobs.GetJobs())
